...
//appRouter.get - send the home page
appRouter.get('/', function (req, res) {
  res.render('home')
})

appRouter.get('/wizard', function (req, res) {
  res.render('wizard')
})
...
appRouter.post('/search', function (req, res) {
  ...
  if ((rating == 'Any') & (cSize == 'Any')) {
    ...
    sql = 'SELECT * FROM centres WHERE suburb=? AND costPerDay < ? AND RATING = ?'
    args = [suburb, cost, rating]
    // sql =
    //   'SELECT * FROM centres WHERE suburb="' +
    //   suburb +
    //   '" AND costPerDay<' +
  ...
  sql.query(sql, arg, (error, results, fields) => {
    if (error) {
...
appRouter.get('/manage', function (req, res) {
...
appRouter.post('/deleteCheck', function (req, res) {
  ...
  sql.query(sql, (err, results, fields) => {
    if (err) {
      console.log(err)
    } else if (results.length === 0) {
      res.render('searchNoResults')
